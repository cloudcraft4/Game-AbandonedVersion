...
#The graphical tiles for the game
wall_tile = 256 
floor_tile = 257
player_tile = 258
orc_tile = 259
...
class Object:
    ...
    def __init__(self, x, y, char, name, color, blocks=False, always_visible=False, fighter=None, ai=None, item=None, equipment=None, summoner=None):
        self.x = x
        self.y = y
        self.char = char
        self.name = name
    ...
    def move(self, dx, dy):
        ...
        for object in objects:  #look for an item in the player's tile
            if self == player and object.x == player.x and object.y == player.y and object.item:
                object.item.pick_up()
...
class Fighter:
    ...
    def ranged_attack(self, target):
        #a simple formula for ranged damage
        damage = self.power + 2 - target.fighter.defense
 
        if damage > 0:
            #make the target take some damage
            message(self.owner.name.capitalize() + ' shoots at ' + target.name + ' for ' + str(damage) + ' hit points.')
            target.fighter.take_damage(damage)
        else:
            message(self.owner.name.capitalize() + ' shoots at ' + target.name + ' but it has no effect!')
    ...
    def take_damage(self, damage):
        ...
        if damage > 0:
            ...
            if self.hp <= 0:
                ...
                if self.owner.summoner is not None:
                    summoner = self.owner.summoner
...
class Archer:
    ...
    def take_turn(self):
        ...
        if libtcod.map_is_in_fov(fov_map, monster.x, monster.y):
            ...
            #move towards player if far away
            if monster.distance_to(player) > self.attack_distance:
                monster.move_towards(player.x, player.y)
 
            elif monster.distance_to(player) <= self.attack_distance and monster.distance_to(player) >= 2:
...
class Necromancer:
    ...
    def take_turn(self):
        ...
        if libtcod.map_is_in_fov(fov_map, monster.x, monster.y):
            ...
            #raise skeletons up to max of 4
            if self.turn <= 1 and self.minions <= 3: 
                if self.minions == 0:
                    self.raisedead(4)
                    self.turn += 1
            ...
            elif monster.distance_to(player) > self.attack_distance:
                monster.move_towards(player.x, player.y)
                self.turn += 1
    ...
    def raisedead(self, num_skeletons):
        #the code for raising skeletons used by necromancers.  
        # I SHOULD CHANGE THIS TO A SUMMON SPELL USED BY ANYONE.  MAYBE UNDER CLASS: SPELLS????  OR CLASS: SKILL???
        caster = self.owner
        
        for skel in range(num_skeletons): 

            for attempt in range(6):
                success = False
                x = (libtcod.random_get_int(0, -2, 2)) + caster.x
                y = (libtcod.random_get_int(0, -2, 2)) + caster.y  
    ...
    for i in range(num_items):
        ...
        if not is_blocked(x, y):
            ...
            if choice == 'heal':
                ...
                item = Object(x, y, shield_tile, 'shield', libtcod.darker_orange, equipment=equipment_component)
...
def render_all():
    global fov_map, wall_tile, floor_tile
...
def skill_menu(header):
    #show a menu with each skill is an option
    if len(skill_list) == 0:
...
def handle_keys():
    ...
    if game_state == 'playing':
        ...
        if key.vk == libtcod.KEY_UP or key.vk == libtcod.KEY_KP8 or key.vk == libtcod.KEY_8:
            ...
            if key_char == 's':
                #show the Player Skills; if an skill is selected, use it
                chosen_item = skill_menu('Press the key next to a skill to use it, or any other to cancel.\n')
                #if chosen_item is not None:
                #    chosen_item.use()
...
def load_game():
    ...
    global map, objects, player, stairs, inventory, game_msgs, game_state, dungeon_level, skill_list
...
def new_game():
    global player, inventory, game_msgs, game_state, dungeon_level, skill_list
...
def main_menu():
    ...
    while not libtcod.console_is_window_closed():
        ...
        libtcod.console_print_ex(0, SCREEN_WIDTH/2, SCREEN_HEIGHT-2, libtcod.BKGND_NONE, libtcod.CENTER, 'By Cloudcraft4')
